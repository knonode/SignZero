...
export function CreateOpinion({ networkId, onCreated }: CreateOpinionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      updateToast(
        toastId,
        `Step 2/2: Approve funding (20 ALGO) + ${numChunks} chunk write${numChunks > 1 ? "s" : ""} (~${feeEstimate}A fees)`,
        "loading",
      );
      ...
      if (flags > 0) {
        ...
        const holdIds = gateAsaHold ? asaHoldIds.filter((id) => id.trim()).map((id) => BigInt(id.trim())) : [];
        const denyIds = gateAsaDeny ? asaDenyIds.filter((id) => id.trim()).map((id) => BigInt(id.trim())) : [];
        ...
        await appClient.send.setGates({
          ...
          boxReferences: [...(gateAsaHold ? ["gate_hold"] : []), ...(gateAsaDeny ? ["gate_deny"] : []), ...(gateNfd ? ["gate_nfd"] : [])],
        });
      }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <p className="text-[var(--text-secondary)] mb-8">Start a new opinion. Requires 20 ALGO funding to cover contract costs.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="px-4 pb-4 space-y-4 border-t border-[var(--border)] pt-4">
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={gateAsaHold}
                    onChange={(e) => setGateAsaHold(e.target.checked)}
                ...
                {gateAsaHold && (
                  <div className="ml-6 space-y-2">
                    {asaHoldIds.map((id, i) => (
                      <div key={i} className="flex gap-2">
                        ...
                        {asaHoldIds.length > 1 && (
                          <button
                            type="button"
                            onClick={() => setAsaHoldIds(asaHoldIds.filter((_, j) => j !== i))}
                            className="px-2 text-[var(--accent-red)] hover:bg-[var(--bg-surface)]"
                          >
                            x
                          </button>
                        )}
                    ...
                    <button
                      type="button"
                      onClick={() => setAsaHoldIds([...asaHoldIds, ""])}
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={gateAsaDeny}
                    onChange={(e) => setGateAsaDeny(e.target.checked)}
                    className="accent-[var(--accent-green)]"
                  />
                ...
                {gateAsaDeny && (
                  <div className="ml-6 space-y-2">
                    {asaDenyIds.map((id, i) => (
                      <div key={i} className="flex gap-2">
                        ...
                        {asaDenyIds.length > 1 && (
                          <button
                            type="button"
                            onClick={() => setAsaDenyIds(asaDenyIds.filter((_, j) => j !== i))}
                            className="px-2 text-[var(--accent-red)] hover:bg-[var(--bg-surface)]"
                          >
                            x
                          </button>
                    ...
                    <button
                      type="button"
                      onClick={() => setAsaDenyIds([...asaDenyIds, ""])}
                      className="text-sm text-[var(--accent-cyan)] hover:underline"
                    >
                      + Add ASA
                    </button>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={gateNfd}
                    onChange={(e) => setGateNfd(e.target.checked)}
                    className="accent-[var(--accent-green)]"
                  />
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={gateAge}
                    onChange={(e) => setGateAge(e.target.checked)}
                    className="accent-[var(--accent-green)]"
                  />
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={gateOnline}
                  onChange={(e) => setGateOnline(e.target.checked)}
                  className="accent-[var(--accent-green)]"
                />
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={gateBalMin}
                    onChange={(e) => setGateBalMin(e.target.checked)}
                    className="accent-[var(--accent-green)]"
                  />
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={gateBalMax}
                    onChange={(e) => setGateBalMax(e.target.checked)}
                    className="accent-[var(--accent-green)]"
                  />
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border)] p-4">
          ...
          <div className="space-y-1 text-sm text-[var(--text-secondary)]">
            ...
            <div className="flex justify-between">
              <span>
                Transaction fees ({Math.ceil(byteLength(text) / 2000) || 1} chunk{Math.ceil(byteLength(text) / 2000) > 1 ? "s" : ""})
              </span>
        ...
        {error && <div className="bg-[var(--bg-surface)] border border-[var(--accent-red)] p-4 text-[var(--accent-red)]">{error}</div>}
