...
Current ways of polling an issue or measuring a sentiment on-chain or off-chain, or a hybrid approach are all vulnerable to this to a bigger or lesser degree.
...
Upon deployment the smart contract mints and ASA with the petition title as `name` and `0` `supply`.
...
The signing of the petition is an opt-in transaction to said ASA. Just as an idea, a thought, it is matterless, intangible, and at the same time completely accesible and the support of it is by comminut members is verifiable, since the supporter holds the zero amount of said ASA in their account together with the 0.1 MBR

Of course, anyone can opt-in to an ASA bypassing the frontend and the eligibility criteria. For that reason, when the petition ends, the smart contract is called again last time, for final verification, substracting those ineligible accounts from total.

Anyone can call for the end of the petition, as long as its assigned number of rounds has passed, and to add a little flair, the lucky caller will receive a small symbolic sum og Algo, that is left there by the author and the unused budget.
